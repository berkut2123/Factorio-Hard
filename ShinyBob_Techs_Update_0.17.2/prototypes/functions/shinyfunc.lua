function setall_icons(technologies_icons)
	if settings.startup["switch_green_purple_belts"] then
		if not settings.startup["switch_green_purple_belts"].value then
			technologies_icons.loaders = {}
			technologies_icons.miniloaders = {}
		end
	end

	for group, objects in pairs(technologies_icons) do
		for mainname, tech in pairs(technologies_icons[group]) do
			local name = tech[1]
			local icon = tech[2]
			local size = tech[3]
			local order = tech[4]
