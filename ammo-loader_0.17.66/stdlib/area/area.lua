...
function Area.area(area) return Area.size(area) end
...
function Area.shrink(area, amount)
	...
	local right_bottom = Position.to_table(area.right_bottom)
	return {
		left_top = {x = left_top.x + amount, y = left_top.y + amount},
		right_bottom = {x = right_bottom.x - amount, y = right_bottom.y - amount}
	}
end
...
function Area.iterate(area)
	...
	function iterator.iterate(area)
		...
		if (area.left_top.y + dy) > area.right_bottom.y then return end
...
function Area.spiral_iterate(area)
	...
	for i = 1, math.max(rx, ry) * math.max(rx, ry) do
		if -(half_x) <= x and x <= half_x and -(half_y) <= y and y <= half_y then table.insert(iterator.list, {x, y}) end
...
function Area.adjust(area) return Area.normalize(area) end
...
---Get array containing only the outermost rectangle of an area.
---@param area Area
function Area.perimeter(area)
	...
	local sides = {
		{x = {value = xLow}, y = {low = yLow, high = yHigh}}, {x = {value = xHigh}, y = {low = yLow, high = yHigh}},
  {x = {low = xLow, high = xHigh}, y = {value = yLow}}, {x = {low = xLow, high = xHigh}, y = {value = yHigh}}
	}
