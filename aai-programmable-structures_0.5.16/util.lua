...
function util.signal_container_get_count(container, signal)
    local table = util.signal_container_get(container, signal)
    if table then return table.count else return nil end
end
...
function util.string_to_number(str, character_multiplier)
    str = ""..str
    local number_string = ""
    local last_char = nil
    for i = 1, #str do
    ...
    if last_char and util.char_to_multiplier[last_char] and character_multiplier ~= false then
...
-- return true/false based on whether this element or any of it's parents have the specificed name
function util.element_is_part_of(element, name)
